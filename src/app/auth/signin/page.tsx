...
// Este componente ya no define <html> o <body>.
// Es un contenedor para el contenido de la página de login.
// El layout global (RootLayout) se aplicará, pero lo modificaremos para que
// no muestre Sidebar/Header en esta ruta específica.
function SignInPageContent() {
  ...
  if (status === "unauthenticated") {
    return (
      <main className="w-full max-w-4xl mx-auto flex items-center justify-center min-h-screen p-4">
        <div className="flex flex-col md:flex-row bg-white dark:bg-slate-800/70 backdrop-blur-md shadow-2xl rounded-xl overflow-hidden w-full">
          {/* Columna Izquierda: Imagen Corporativa */}
          <div className="w-full md:w-1/2 hidden md:block relative min-h-[400px] md:min-h-[550px]">
            {/* IMPORTANTE: Asegúrate de que la imagen exista en public/images/accesspoint-login-bg.jpg */}
            <Image
              src="/images/accesspoint-login-bg.jpg" 
              alt="Imagen Corporativa AccessPoint Control"
              layout="fill"
              objectFit="cover"
              priority
              onError={(e) => {
                // Fallback si la imagen principal no carga
                (e.target as HTMLImageElement).style.display = 'none'; // Ocultar la imagen rota
                // Considera mostrar un color de fondo o un placeholder más simple aquí
                // para evitar bucles si el placeholder también falla.
                // O simplemente no hacer nada y que el espacio quede vacío o con el alt text.
...
// La página ahora exporta directamente el contenido.
// El RootLayout aplicará el <html> y <body>.
// Necesitaremos modificar RootLayout para que no muestre Sidebar/Header en esta ruta.
export default function SignInPageWrapper() {
    // El SignInPageLayout se elimina como componente exportado que renderiza <html> y <body>
    // Ahora SignInPageContent maneja la lógica y el marcado interno.
    // El RootLayout de Next.js proveerá el <html> y <body>.
    // Esta página (auth/signin/page.tsx) simplemente devuelve el contenido que debe ir DENTRO del body.
