...
export class EquipoInventarioService {
  ...
  static async createEquipoInventario(data: EquipoInventarioCreateInput): Promise<EquipoInventario> {
    try {
      ...
      if (error instanceof Error && 'issues' in error) { // ZodError
        throw new Error("Error de validación al crear equipo: " + (error as any).issues.map((i: any) => i.message).join(", "));
      }
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') { // Unique constraint failed (e.g., on identificadorUnico)
          throw new Error("Error al crear equipo: El identificador único ya existe.");
        }
      }
  ...
  static async updateEquipoInventario(data: EquipoInventarioUpdateInput): Promise<EquipoInventario> {
    try {
      ...
      if (error instanceof Error && 'issues' in error) { // ZodError
        throw new Error("Error de validación al actualizar equipo: " + (error as any).issues.map((i: any) => i.message).join(", "));
      }
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') { // Unique constraint failed (e.g., on identificadorUnico)
          throw new Error("Error al actualizar equipo: El identificador único ya existe.");
        }
      }
  ...
  static async deleteEquipoInventario(id: string): Promise<{ success: boolean; message?: string }> {
    try {
      await prisma.$transaction(async (tx) => {
        ...
        if (prestamosCount > 0) {
          throw new Error("No se puede eliminar el equipo porque tiene registros de préstamos asociados. Por favor, asegúrate de que no haya préstamos activos y gestiona los históricos si es necesario.");
        }
      ...
      return { success: false, message: error.message || "Error al eliminar el equipo de inventario." };
    }
  }
}
