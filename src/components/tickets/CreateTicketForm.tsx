...
const ticketFormSchema = z.object({
  ...
  titulo: z.string().min(1, "El título es obligatorio.").max(200, "Título demasiado largo."),
...
export const CreateTicketForm = React.forwardRef<HTMLFormElement, CreateTicketFormProps>(({
  ...
  const {
    ...
    defaultValues: {
      fechaCreacion: stashedData?.get('fechaCreacion')?.toString() || getLocalDateTimeString(),
            ...
            <div className="md:col-span-2 space-y-1.5">
              ...
              <Controller control={control} name="empresaId" render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    <SelectTrigger id="empresaId" className={errors.empresaId ? "border-destructive" : ""}><SelectValue placeholder="Seleccione una Empresa" /></SelectTrigger>
            ...
            {selectedEmpresaId && (
              <div className="md:col-span-2 space-y-1.5">
                ...
                <SucursalSelector
                  ...
                  onSucursalSelect={(sucursalId) => {
                    setValue('sucursalId', sucursalId, { shouldValidate: true });
                  }}
                />
                {errors.sucursalId && <p className="text-destructive text-xs mt-1">{errors.sucursalId.message}</p>}
              </div>
            )}
